...
{% asset_img 程序计数器执行图.png %}

{% asset_img 栈中main执行第一步图.png %}

3a+b操作

{% asset_img 栈中main执行第二步图.png %}
...
{% asset_img 对象头字节数分布图.png %}

下图是结合栈以及方法区画了一个个人理解和参考的图

{% asset_img 个人理解堆栈操作图.png %}
...
{% asset_img 标记清除回收前图.png %}

回收后：

{% asset_img 标记清除回收后图.png %}
...
{% asset_img 标记复制回收前图.png %}

回收后：

{% asset_img 标记复制回收后图.png %}
...
{% asset_img 标记整理回收前图.png %}

回收后：

{% asset_img 标记整理回收后图.png %}
...
{% asset_img Serial组合Serial Old运行图.png %}
...
{% asset_img ParNew组合SerialOld运行图.png %}
...
{% asset_img ParallelScavenge组合ParallelOld运行图.png %}
...
由于new对象，所以这种方式会在堆和字符串常量池中都拥有这个对象，没有就会创建，最后再返回**堆内存中的对象引用。str指向的是内存中的对象引用**
...
{% asset_img stringbuilder反编译字节码图.png %}
